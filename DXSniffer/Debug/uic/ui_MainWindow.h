/********************************************************************************
** Form generated from reading UI file 'MainWindow.ui'
**
** Created by: Qt User Interface Compiler version 5.7.1
**
** WARNING! All changes made in this file will be lost when recompiling UI file!
********************************************************************************/

#ifndef UI_MAINWINDOW_H
#define UI_MAINWINDOW_H

#include <QtCore/QVariant>
#include <QtWidgets/QAction>
#include <QtWidgets/QApplication>
#include <QtWidgets/QButtonGroup>
#include <QtWidgets/QGridLayout>
#include <QtWidgets/QGroupBox>
#include <QtWidgets/QHBoxLayout>
#include <QtWidgets/QHeaderView>
#include <QtWidgets/QLabel>
#include <QtWidgets/QLineEdit>
#include <QtWidgets/QMainWindow>
#include <QtWidgets/QPushButton>
#include <QtWidgets/QRadioButton>
#include <QtWidgets/QScrollArea>
#include <QtWidgets/QSpacerItem>
#include <QtWidgets/QTabWidget>
#include <QtWidgets/QTableWidget>
#include <QtWidgets/QVBoxLayout>
#include <QtWidgets/QWidget>

QT_BEGIN_NAMESPACE

class Ui_MainWindowClass
{
public:
    QWidget *centralWidget;
    QGridLayout *gridLayout_2;
    QTabWidget *tabWidget;
    QWidget *tab;
    QGridLayout *gridLayout_3;
    QGridLayout *gridLayout;
    QGroupBox *groupBox_device;
    QVBoxLayout *verticalLayout_2;
    QScrollArea *scrollArea;
    QWidget *scrollAreaWidgetContents;
    QVBoxLayout *verticalLayout_4;
    QVBoxLayout *verticalLayout_device;
    QPushButton *submitDevice;
    QGroupBox *groupBox_packet;
    QVBoxLayout *verticalLayout_3;
    QScrollArea *scrollArea_2;
    QWidget *scrollAreaWidgetContents_2;
    QVBoxLayout *verticalLayout_6;
    QTableWidget *tableWidget;
    QWidget *tab_2;
    QGridLayout *gridLayout_5;
    QGridLayout *gridLayout_4;
    QGroupBox *groupBox;
    QVBoxLayout *verticalLayout;
    QScrollArea *scrollArea_3;
    QWidget *scrollAreaWidgetContents_3;
    QVBoxLayout *verticalLayout_7;
    QVBoxLayout *verticalLayout_device2;
    QGroupBox *groupBox_3;
    QHBoxLayout *horizontalLayout_3;
    QHBoxLayout *horizontalLayout_2;
    QGroupBox *groupBox_4;
    QVBoxLayout *verticalLayout_5;
    QHBoxLayout *horizontalLayout;
    QLabel *label;
    QRadioButton *ARPTypeRB_Rq;
    QRadioButton *ARPTypeRB_Rsp;
    QSpacerItem *horizontalSpacer;
    QHBoxLayout *horizontalLayout_4;
    QLabel *label_2;
    QLineEdit *lineEdit;
    QLabel *label_3;
    QLineEdit *lineEdit_2;
    QLabel *label_4;
    QLineEdit *lineEdit_3;
    QLabel *label_5;
    QLineEdit *lineEdit_4;
    QLabel *label_6;
    QLineEdit *lineEdit_5;
    QLabel *label_7;
    QLineEdit *lineEdit_6;
    QHBoxLayout *horizontalLayout_5;
    QLabel *label_8;
    QLineEdit *lineEdit_7;
    QLabel *label_9;
    QLineEdit *lineEdit_8;
    QLabel *label_10;
    QLineEdit *lineEdit_9;
    QLabel *label_11;
    QLineEdit *lineEdit_10;
    QLabel *label_12;
    QLineEdit *lineEdit_11;
    QLabel *label_13;
    QLineEdit *lineEdit_12;
    QHBoxLayout *horizontalLayout_14;
    QLabel *label_44;
    QLineEdit *lineEdit_43;
    QLabel *label_45;
    QLineEdit *lineEdit_44;
    QLabel *label_46;
    QLineEdit *lineEdit_45;
    QLabel *label_47;
    QLineEdit *lineEdit_46;
    QLabel *label_48;
    QLineEdit *lineEdit_47;
    QLabel *label_49;
    QLineEdit *lineEdit_48;
    QHBoxLayout *horizontalLayout_6;
    QLabel *label_14;
    QLineEdit *lineEdit_13;
    QLabel *label_15;
    QLineEdit *lineEdit_14;
    QLabel *label_16;
    QLineEdit *lineEdit_15;
    QLabel *label_17;
    QLineEdit *lineEdit_16;
    QSpacerItem *horizontalSpacer_9;
    QHBoxLayout *horizontalLayout_7;
    QLabel *label_18;
    QLineEdit *lineEdit_17;
    QLabel *label_19;
    QLineEdit *lineEdit_18;
    QLabel *label_20;
    QLineEdit *lineEdit_19;
    QLabel *label_21;
    QLineEdit *lineEdit_20;
    QSpacerItem *horizontalSpacer_8;
    QPushButton *SendPacketBtn_ARP;
    QGroupBox *groupBox_5;
    QVBoxLayout *verticalLayout_8;
    QHBoxLayout *horizontalLayout_8;
    QLabel *label_22;
    QLineEdit *lineEdit_21;
    QLabel *label_23;
    QLineEdit *lineEdit_22;
    QLabel *label_24;
    QLineEdit *lineEdit_23;
    QLabel *label_25;
    QLineEdit *lineEdit_24;
    QLabel *label_26;
    QLineEdit *lineEdit_25;
    QLabel *label_27;
    QLineEdit *lineEdit_26;
    QSpacerItem *horizontalSpacer_7;
    QHBoxLayout *horizontalLayout_9;
    QLabel *label_28;
    QLineEdit *lineEdit_27;
    QLabel *label_29;
    QLineEdit *lineEdit_28;
    QLabel *label_30;
    QLineEdit *lineEdit_29;
    QLabel *label_31;
    QLineEdit *lineEdit_30;
    QLabel *label_32;
    QLineEdit *lineEdit_31;
    QLabel *label_33;
    QLineEdit *lineEdit_32;
    QSpacerItem *horizontalSpacer_6;
    QHBoxLayout *horizontalLayout_10;
    QLabel *label_34;
    QLineEdit *lineEdit_33;
    QLabel *label_35;
    QLineEdit *lineEdit_34;
    QLabel *label_36;
    QLineEdit *lineEdit_35;
    QLabel *label_37;
    QLineEdit *lineEdit_36;
    QSpacerItem *horizontalSpacer_5;
    QHBoxLayout *horizontalLayout_11;
    QLabel *label_38;
    QLineEdit *lineEdit_37;
    QLabel *label_39;
    QLineEdit *lineEdit_38;
    QLabel *label_40;
    QLineEdit *lineEdit_39;
    QLabel *label_41;
    QLineEdit *lineEdit_40;
    QSpacerItem *horizontalSpacer_4;
    QHBoxLayout *horizontalLayout_12;
    QLabel *label_42;
    QLineEdit *lineEdit_41;
    QSpacerItem *horizontalSpacer_2;
    QLabel *label_43;
    QLineEdit *lineEdit_42;
    QSpacerItem *horizontalSpacer_3;
    QHBoxLayout *horizontalLayout_13;
    QLabel *label_50;
    QLineEdit *lineEdit_49;
    QSpacerItem *horizontalSpacer_10;
    QLabel *label_51;
    QLineEdit *lineEdit_50;
    QSpacerItem *horizontalSpacer_11;
    QHBoxLayout *horizontalLayout_16;
    QLabel *label_53;
    QLineEdit *lineEdit_51;
    QSpacerItem *horizontalSpacer_13;
    QLabel *label_55;
    QLineEdit *lineEdit_52;
    QSpacerItem *horizontalSpacer_12;
    QPushButton *SendPacketBtn_TCP;

    void setupUi(QMainWindow *MainWindowClass)
    {
        if (MainWindowClass->objectName().isEmpty())
            MainWindowClass->setObjectName(QStringLiteral("MainWindowClass"));
        MainWindowClass->resize(925, 780);
        QSizePolicy sizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
        sizePolicy.setHorizontalStretch(0);
        sizePolicy.setVerticalStretch(0);
        sizePolicy.setHeightForWidth(MainWindowClass->sizePolicy().hasHeightForWidth());
        MainWindowClass->setSizePolicy(sizePolicy);
        MainWindowClass->setMinimumSize(QSize(20, 20));
        MainWindowClass->setMaximumSize(QSize(65536, 65536));
        MainWindowClass->setSizeIncrement(QSize(500, 500));
        MainWindowClass->setAutoFillBackground(false);
        MainWindowClass->setAnimated(true);
        centralWidget = new QWidget(MainWindowClass);
        centralWidget->setObjectName(QStringLiteral("centralWidget"));
        centralWidget->setEnabled(true);
        QSizePolicy sizePolicy1(QSizePolicy::Expanding, QSizePolicy::Expanding);
        sizePolicy1.setHorizontalStretch(0);
        sizePolicy1.setVerticalStretch(0);
        sizePolicy1.setHeightForWidth(centralWidget->sizePolicy().hasHeightForWidth());
        centralWidget->setSizePolicy(sizePolicy1);
        centralWidget->setAcceptDrops(false);
        gridLayout_2 = new QGridLayout(centralWidget);
        gridLayout_2->setSpacing(6);
        gridLayout_2->setContentsMargins(11, 11, 11, 11);
        gridLayout_2->setObjectName(QStringLiteral("gridLayout_2"));
        tabWidget = new QTabWidget(centralWidget);
        tabWidget->setObjectName(QStringLiteral("tabWidget"));
        tab = new QWidget();
        tab->setObjectName(QStringLiteral("tab"));
        gridLayout_3 = new QGridLayout(tab);
        gridLayout_3->setSpacing(6);
        gridLayout_3->setContentsMargins(11, 11, 11, 11);
        gridLayout_3->setObjectName(QStringLiteral("gridLayout_3"));
        gridLayout = new QGridLayout();
        gridLayout->setSpacing(6);
        gridLayout->setObjectName(QStringLiteral("gridLayout"));
        gridLayout->setContentsMargins(0, 0, 0, 0);
        groupBox_device = new QGroupBox(tab);
        groupBox_device->setObjectName(QStringLiteral("groupBox_device"));
        groupBox_device->setEnabled(true);
        QSizePolicy sizePolicy2(QSizePolicy::Expanding, QSizePolicy::Fixed);
        sizePolicy2.setHorizontalStretch(0);
        sizePolicy2.setVerticalStretch(0);
        sizePolicy2.setHeightForWidth(groupBox_device->sizePolicy().hasHeightForWidth());
        groupBox_device->setSizePolicy(sizePolicy2);
        groupBox_device->setMinimumSize(QSize(0, 200));
        verticalLayout_2 = new QVBoxLayout(groupBox_device);
        verticalLayout_2->setSpacing(6);
        verticalLayout_2->setContentsMargins(11, 11, 11, 11);
        verticalLayout_2->setObjectName(QStringLiteral("verticalLayout_2"));
        scrollArea = new QScrollArea(groupBox_device);
        scrollArea->setObjectName(QStringLiteral("scrollArea"));
        scrollArea->setWidgetResizable(true);
        scrollAreaWidgetContents = new QWidget();
        scrollAreaWidgetContents->setObjectName(QStringLiteral("scrollAreaWidgetContents"));
        scrollAreaWidgetContents->setGeometry(QRect(0, 0, 847, 124));
        verticalLayout_4 = new QVBoxLayout(scrollAreaWidgetContents);
        verticalLayout_4->setSpacing(6);
        verticalLayout_4->setContentsMargins(11, 11, 11, 11);
        verticalLayout_4->setObjectName(QStringLiteral("verticalLayout_4"));
        verticalLayout_device = new QVBoxLayout();
        verticalLayout_device->setSpacing(7);
        verticalLayout_device->setObjectName(QStringLiteral("verticalLayout_device"));
        verticalLayout_device->setContentsMargins(-1, 0, -1, -1);

        verticalLayout_4->addLayout(verticalLayout_device);

        scrollArea->setWidget(scrollAreaWidgetContents);

        verticalLayout_2->addWidget(scrollArea);

        submitDevice = new QPushButton(groupBox_device);
        submitDevice->setObjectName(QStringLiteral("submitDevice"));
        QSizePolicy sizePolicy3(QSizePolicy::Minimum, QSizePolicy::Fixed);
        sizePolicy3.setHorizontalStretch(0);
        sizePolicy3.setVerticalStretch(0);
        sizePolicy3.setHeightForWidth(submitDevice->sizePolicy().hasHeightForWidth());
        submitDevice->setSizePolicy(sizePolicy3);
        submitDevice->setBaseSize(QSize(0, 0));

        verticalLayout_2->addWidget(submitDevice);


        gridLayout->addWidget(groupBox_device, 2, 0, 1, 1);

        groupBox_packet = new QGroupBox(tab);
        groupBox_packet->setObjectName(QStringLiteral("groupBox_packet"));
        verticalLayout_3 = new QVBoxLayout(groupBox_packet);
        verticalLayout_3->setSpacing(6);
        verticalLayout_3->setContentsMargins(11, 11, 11, 11);
        verticalLayout_3->setObjectName(QStringLiteral("verticalLayout_3"));
        scrollArea_2 = new QScrollArea(groupBox_packet);
        scrollArea_2->setObjectName(QStringLiteral("scrollArea_2"));
        scrollArea_2->setWidgetResizable(true);
        scrollAreaWidgetContents_2 = new QWidget();
        scrollAreaWidgetContents_2->setObjectName(QStringLiteral("scrollAreaWidgetContents_2"));
        scrollAreaWidgetContents_2->setGeometry(QRect(0, 0, 847, 458));
        verticalLayout_6 = new QVBoxLayout(scrollAreaWidgetContents_2);
        verticalLayout_6->setSpacing(6);
        verticalLayout_6->setContentsMargins(11, 11, 11, 11);
        verticalLayout_6->setObjectName(QStringLiteral("verticalLayout_6"));
        tableWidget = new QTableWidget(scrollAreaWidgetContents_2);
        tableWidget->setObjectName(QStringLiteral("tableWidget"));

        verticalLayout_6->addWidget(tableWidget);

        scrollArea_2->setWidget(scrollAreaWidgetContents_2);

        verticalLayout_3->addWidget(scrollArea_2);


        gridLayout->addWidget(groupBox_packet, 3, 0, 1, 1);


        gridLayout_3->addLayout(gridLayout, 0, 0, 1, 1);

        tabWidget->addTab(tab, QString());
        tab_2 = new QWidget();
        tab_2->setObjectName(QStringLiteral("tab_2"));
        gridLayout_5 = new QGridLayout(tab_2);
        gridLayout_5->setSpacing(6);
        gridLayout_5->setContentsMargins(11, 11, 11, 11);
        gridLayout_5->setObjectName(QStringLiteral("gridLayout_5"));
        gridLayout_4 = new QGridLayout();
        gridLayout_4->setSpacing(6);
        gridLayout_4->setObjectName(QStringLiteral("gridLayout_4"));
        groupBox = new QGroupBox(tab_2);
        groupBox->setObjectName(QStringLiteral("groupBox"));
        QSizePolicy sizePolicy4(QSizePolicy::Preferred, QSizePolicy::Fixed);
        sizePolicy4.setHorizontalStretch(0);
        sizePolicy4.setVerticalStretch(0);
        sizePolicy4.setHeightForWidth(groupBox->sizePolicy().hasHeightForWidth());
        groupBox->setSizePolicy(sizePolicy4);
        groupBox->setMinimumSize(QSize(0, 200));
        verticalLayout = new QVBoxLayout(groupBox);
        verticalLayout->setSpacing(6);
        verticalLayout->setContentsMargins(11, 11, 11, 11);
        verticalLayout->setObjectName(QStringLiteral("verticalLayout"));
        scrollArea_3 = new QScrollArea(groupBox);
        scrollArea_3->setObjectName(QStringLiteral("scrollArea_3"));
        scrollArea_3->setWidgetResizable(true);
        scrollAreaWidgetContents_3 = new QWidget();
        scrollAreaWidgetContents_3->setObjectName(QStringLiteral("scrollAreaWidgetContents_3"));
        scrollAreaWidgetContents_3->setGeometry(QRect(0, 0, 847, 159));
        verticalLayout_7 = new QVBoxLayout(scrollAreaWidgetContents_3);
        verticalLayout_7->setSpacing(6);
        verticalLayout_7->setContentsMargins(11, 11, 11, 11);
        verticalLayout_7->setObjectName(QStringLiteral("verticalLayout_7"));
        verticalLayout_device2 = new QVBoxLayout();
        verticalLayout_device2->setSpacing(6);
        verticalLayout_device2->setObjectName(QStringLiteral("verticalLayout_device2"));

        verticalLayout_7->addLayout(verticalLayout_device2);

        scrollArea_3->setWidget(scrollAreaWidgetContents_3);

        verticalLayout->addWidget(scrollArea_3);


        gridLayout_4->addWidget(groupBox, 1, 0, 1, 1);

        groupBox_3 = new QGroupBox(tab_2);
        groupBox_3->setObjectName(QStringLiteral("groupBox_3"));
        horizontalLayout_3 = new QHBoxLayout(groupBox_3);
        horizontalLayout_3->setSpacing(6);
        horizontalLayout_3->setContentsMargins(11, 11, 11, 11);
        horizontalLayout_3->setObjectName(QStringLiteral("horizontalLayout_3"));
        horizontalLayout_2 = new QHBoxLayout();
        horizontalLayout_2->setSpacing(6);
        horizontalLayout_2->setObjectName(QStringLiteral("horizontalLayout_2"));
        groupBox_4 = new QGroupBox(groupBox_3);
        groupBox_4->setObjectName(QStringLiteral("groupBox_4"));
        verticalLayout_5 = new QVBoxLayout(groupBox_4);
        verticalLayout_5->setSpacing(6);
        verticalLayout_5->setContentsMargins(11, 11, 11, 11);
        verticalLayout_5->setObjectName(QStringLiteral("verticalLayout_5"));
        horizontalLayout = new QHBoxLayout();
        horizontalLayout->setSpacing(6);
        horizontalLayout->setObjectName(QStringLiteral("horizontalLayout"));
        label = new QLabel(groupBox_4);
        label->setObjectName(QStringLiteral("label"));
        sizePolicy.setHeightForWidth(label->sizePolicy().hasHeightForWidth());
        label->setSizePolicy(sizePolicy);

        horizontalLayout->addWidget(label);

        ARPTypeRB_Rq = new QRadioButton(groupBox_4);
        ARPTypeRB_Rq->setObjectName(QStringLiteral("ARPTypeRB_Rq"));
        sizePolicy3.setHeightForWidth(ARPTypeRB_Rq->sizePolicy().hasHeightForWidth());
        ARPTypeRB_Rq->setSizePolicy(sizePolicy3);

        horizontalLayout->addWidget(ARPTypeRB_Rq);

        ARPTypeRB_Rsp = new QRadioButton(groupBox_4);
        ARPTypeRB_Rsp->setObjectName(QStringLiteral("ARPTypeRB_Rsp"));
        sizePolicy3.setHeightForWidth(ARPTypeRB_Rsp->sizePolicy().hasHeightForWidth());
        ARPTypeRB_Rsp->setSizePolicy(sizePolicy3);

        horizontalLayout->addWidget(ARPTypeRB_Rsp);

        horizontalSpacer = new QSpacerItem(100, 20, QSizePolicy::Fixed, QSizePolicy::Minimum);

        horizontalLayout->addItem(horizontalSpacer);


        verticalLayout_5->addLayout(horizontalLayout);

        horizontalLayout_4 = new QHBoxLayout();
        horizontalLayout_4->setSpacing(6);
        horizontalLayout_4->setObjectName(QStringLiteral("horizontalLayout_4"));
        label_2 = new QLabel(groupBox_4);
        label_2->setObjectName(QStringLiteral("label_2"));
        QSizePolicy sizePolicy5(QSizePolicy::Fixed, QSizePolicy::Minimum);
        sizePolicy5.setHorizontalStretch(0);
        sizePolicy5.setVerticalStretch(0);
        sizePolicy5.setHeightForWidth(label_2->sizePolicy().hasHeightForWidth());
        label_2->setSizePolicy(sizePolicy5);

        horizontalLayout_4->addWidget(label_2);

        lineEdit = new QLineEdit(groupBox_4);
        lineEdit->setObjectName(QStringLiteral("lineEdit"));
        sizePolicy.setHeightForWidth(lineEdit->sizePolicy().hasHeightForWidth());
        lineEdit->setSizePolicy(sizePolicy);
        lineEdit->setMinimumSize(QSize(20, 0));
        lineEdit->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit);

        label_3 = new QLabel(groupBox_4);
        label_3->setObjectName(QStringLiteral("label_3"));
        QSizePolicy sizePolicy6(QSizePolicy::Fixed, QSizePolicy::Preferred);
        sizePolicy6.setHorizontalStretch(0);
        sizePolicy6.setVerticalStretch(0);
        sizePolicy6.setHeightForWidth(label_3->sizePolicy().hasHeightForWidth());
        label_3->setSizePolicy(sizePolicy6);

        horizontalLayout_4->addWidget(label_3);

        lineEdit_2 = new QLineEdit(groupBox_4);
        lineEdit_2->setObjectName(QStringLiteral("lineEdit_2"));
        sizePolicy.setHeightForWidth(lineEdit_2->sizePolicy().hasHeightForWidth());
        lineEdit_2->setSizePolicy(sizePolicy);
        lineEdit_2->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit_2);

        label_4 = new QLabel(groupBox_4);
        label_4->setObjectName(QStringLiteral("label_4"));
        sizePolicy6.setHeightForWidth(label_4->sizePolicy().hasHeightForWidth());
        label_4->setSizePolicy(sizePolicy6);

        horizontalLayout_4->addWidget(label_4);

        lineEdit_3 = new QLineEdit(groupBox_4);
        lineEdit_3->setObjectName(QStringLiteral("lineEdit_3"));
        sizePolicy.setHeightForWidth(lineEdit_3->sizePolicy().hasHeightForWidth());
        lineEdit_3->setSizePolicy(sizePolicy);
        lineEdit_3->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit_3);

        label_5 = new QLabel(groupBox_4);
        label_5->setObjectName(QStringLiteral("label_5"));
        sizePolicy6.setHeightForWidth(label_5->sizePolicy().hasHeightForWidth());
        label_5->setSizePolicy(sizePolicy6);

        horizontalLayout_4->addWidget(label_5);

        lineEdit_4 = new QLineEdit(groupBox_4);
        lineEdit_4->setObjectName(QStringLiteral("lineEdit_4"));
        sizePolicy.setHeightForWidth(lineEdit_4->sizePolicy().hasHeightForWidth());
        lineEdit_4->setSizePolicy(sizePolicy);
        lineEdit_4->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit_4);

        label_6 = new QLabel(groupBox_4);
        label_6->setObjectName(QStringLiteral("label_6"));
        sizePolicy6.setHeightForWidth(label_6->sizePolicy().hasHeightForWidth());
        label_6->setSizePolicy(sizePolicy6);

        horizontalLayout_4->addWidget(label_6);

        lineEdit_5 = new QLineEdit(groupBox_4);
        lineEdit_5->setObjectName(QStringLiteral("lineEdit_5"));
        sizePolicy.setHeightForWidth(lineEdit_5->sizePolicy().hasHeightForWidth());
        lineEdit_5->setSizePolicy(sizePolicy);
        lineEdit_5->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit_5);

        label_7 = new QLabel(groupBox_4);
        label_7->setObjectName(QStringLiteral("label_7"));
        sizePolicy6.setHeightForWidth(label_7->sizePolicy().hasHeightForWidth());
        label_7->setSizePolicy(sizePolicy6);

        horizontalLayout_4->addWidget(label_7);

        lineEdit_6 = new QLineEdit(groupBox_4);
        lineEdit_6->setObjectName(QStringLiteral("lineEdit_6"));
        sizePolicy.setHeightForWidth(lineEdit_6->sizePolicy().hasHeightForWidth());
        lineEdit_6->setSizePolicy(sizePolicy);
        lineEdit_6->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_4->addWidget(lineEdit_6);


        verticalLayout_5->addLayout(horizontalLayout_4);

        horizontalLayout_5 = new QHBoxLayout();
        horizontalLayout_5->setSpacing(6);
        horizontalLayout_5->setObjectName(QStringLiteral("horizontalLayout_5"));
        label_8 = new QLabel(groupBox_4);
        label_8->setObjectName(QStringLiteral("label_8"));
        sizePolicy5.setHeightForWidth(label_8->sizePolicy().hasHeightForWidth());
        label_8->setSizePolicy(sizePolicy5);

        horizontalLayout_5->addWidget(label_8);

        lineEdit_7 = new QLineEdit(groupBox_4);
        lineEdit_7->setObjectName(QStringLiteral("lineEdit_7"));
        sizePolicy.setHeightForWidth(lineEdit_7->sizePolicy().hasHeightForWidth());
        lineEdit_7->setSizePolicy(sizePolicy);
        lineEdit_7->setMinimumSize(QSize(20, 0));
        lineEdit_7->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_7);

        label_9 = new QLabel(groupBox_4);
        label_9->setObjectName(QStringLiteral("label_9"));
        sizePolicy6.setHeightForWidth(label_9->sizePolicy().hasHeightForWidth());
        label_9->setSizePolicy(sizePolicy6);

        horizontalLayout_5->addWidget(label_9);

        lineEdit_8 = new QLineEdit(groupBox_4);
        lineEdit_8->setObjectName(QStringLiteral("lineEdit_8"));
        sizePolicy.setHeightForWidth(lineEdit_8->sizePolicy().hasHeightForWidth());
        lineEdit_8->setSizePolicy(sizePolicy);
        lineEdit_8->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_8);

        label_10 = new QLabel(groupBox_4);
        label_10->setObjectName(QStringLiteral("label_10"));
        sizePolicy6.setHeightForWidth(label_10->sizePolicy().hasHeightForWidth());
        label_10->setSizePolicy(sizePolicy6);

        horizontalLayout_5->addWidget(label_10);

        lineEdit_9 = new QLineEdit(groupBox_4);
        lineEdit_9->setObjectName(QStringLiteral("lineEdit_9"));
        sizePolicy.setHeightForWidth(lineEdit_9->sizePolicy().hasHeightForWidth());
        lineEdit_9->setSizePolicy(sizePolicy);
        lineEdit_9->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_9);

        label_11 = new QLabel(groupBox_4);
        label_11->setObjectName(QStringLiteral("label_11"));
        sizePolicy6.setHeightForWidth(label_11->sizePolicy().hasHeightForWidth());
        label_11->setSizePolicy(sizePolicy6);

        horizontalLayout_5->addWidget(label_11);

        lineEdit_10 = new QLineEdit(groupBox_4);
        lineEdit_10->setObjectName(QStringLiteral("lineEdit_10"));
        sizePolicy.setHeightForWidth(lineEdit_10->sizePolicy().hasHeightForWidth());
        lineEdit_10->setSizePolicy(sizePolicy);
        lineEdit_10->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_10);

        label_12 = new QLabel(groupBox_4);
        label_12->setObjectName(QStringLiteral("label_12"));
        sizePolicy6.setHeightForWidth(label_12->sizePolicy().hasHeightForWidth());
        label_12->setSizePolicy(sizePolicy6);

        horizontalLayout_5->addWidget(label_12);

        lineEdit_11 = new QLineEdit(groupBox_4);
        lineEdit_11->setObjectName(QStringLiteral("lineEdit_11"));
        sizePolicy.setHeightForWidth(lineEdit_11->sizePolicy().hasHeightForWidth());
        lineEdit_11->setSizePolicy(sizePolicy);
        lineEdit_11->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_11);

        label_13 = new QLabel(groupBox_4);
        label_13->setObjectName(QStringLiteral("label_13"));
        sizePolicy6.setHeightForWidth(label_13->sizePolicy().hasHeightForWidth());
        label_13->setSizePolicy(sizePolicy6);

        horizontalLayout_5->addWidget(label_13);

        lineEdit_12 = new QLineEdit(groupBox_4);
        lineEdit_12->setObjectName(QStringLiteral("lineEdit_12"));
        sizePolicy.setHeightForWidth(lineEdit_12->sizePolicy().hasHeightForWidth());
        lineEdit_12->setSizePolicy(sizePolicy);
        lineEdit_12->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_5->addWidget(lineEdit_12);


        verticalLayout_5->addLayout(horizontalLayout_5);

        horizontalLayout_14 = new QHBoxLayout();
        horizontalLayout_14->setSpacing(6);
        horizontalLayout_14->setObjectName(QStringLiteral("horizontalLayout_14"));
        label_44 = new QLabel(groupBox_4);
        label_44->setObjectName(QStringLiteral("label_44"));
        sizePolicy5.setHeightForWidth(label_44->sizePolicy().hasHeightForWidth());
        label_44->setSizePolicy(sizePolicy5);

        horizontalLayout_14->addWidget(label_44);

        lineEdit_43 = new QLineEdit(groupBox_4);
        lineEdit_43->setObjectName(QStringLiteral("lineEdit_43"));
        sizePolicy.setHeightForWidth(lineEdit_43->sizePolicy().hasHeightForWidth());
        lineEdit_43->setSizePolicy(sizePolicy);
        lineEdit_43->setMinimumSize(QSize(20, 0));
        lineEdit_43->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_43);

        label_45 = new QLabel(groupBox_4);
        label_45->setObjectName(QStringLiteral("label_45"));
        sizePolicy6.setHeightForWidth(label_45->sizePolicy().hasHeightForWidth());
        label_45->setSizePolicy(sizePolicy6);

        horizontalLayout_14->addWidget(label_45);

        lineEdit_44 = new QLineEdit(groupBox_4);
        lineEdit_44->setObjectName(QStringLiteral("lineEdit_44"));
        sizePolicy.setHeightForWidth(lineEdit_44->sizePolicy().hasHeightForWidth());
        lineEdit_44->setSizePolicy(sizePolicy);
        lineEdit_44->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_44);

        label_46 = new QLabel(groupBox_4);
        label_46->setObjectName(QStringLiteral("label_46"));
        sizePolicy6.setHeightForWidth(label_46->sizePolicy().hasHeightForWidth());
        label_46->setSizePolicy(sizePolicy6);

        horizontalLayout_14->addWidget(label_46);

        lineEdit_45 = new QLineEdit(groupBox_4);
        lineEdit_45->setObjectName(QStringLiteral("lineEdit_45"));
        sizePolicy.setHeightForWidth(lineEdit_45->sizePolicy().hasHeightForWidth());
        lineEdit_45->setSizePolicy(sizePolicy);
        lineEdit_45->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_45);

        label_47 = new QLabel(groupBox_4);
        label_47->setObjectName(QStringLiteral("label_47"));
        sizePolicy6.setHeightForWidth(label_47->sizePolicy().hasHeightForWidth());
        label_47->setSizePolicy(sizePolicy6);

        horizontalLayout_14->addWidget(label_47);

        lineEdit_46 = new QLineEdit(groupBox_4);
        lineEdit_46->setObjectName(QStringLiteral("lineEdit_46"));
        sizePolicy.setHeightForWidth(lineEdit_46->sizePolicy().hasHeightForWidth());
        lineEdit_46->setSizePolicy(sizePolicy);
        lineEdit_46->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_46);

        label_48 = new QLabel(groupBox_4);
        label_48->setObjectName(QStringLiteral("label_48"));
        sizePolicy6.setHeightForWidth(label_48->sizePolicy().hasHeightForWidth());
        label_48->setSizePolicy(sizePolicy6);

        horizontalLayout_14->addWidget(label_48);

        lineEdit_47 = new QLineEdit(groupBox_4);
        lineEdit_47->setObjectName(QStringLiteral("lineEdit_47"));
        sizePolicy.setHeightForWidth(lineEdit_47->sizePolicy().hasHeightForWidth());
        lineEdit_47->setSizePolicy(sizePolicy);
        lineEdit_47->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_47);

        label_49 = new QLabel(groupBox_4);
        label_49->setObjectName(QStringLiteral("label_49"));
        sizePolicy6.setHeightForWidth(label_49->sizePolicy().hasHeightForWidth());
        label_49->setSizePolicy(sizePolicy6);

        horizontalLayout_14->addWidget(label_49);

        lineEdit_48 = new QLineEdit(groupBox_4);
        lineEdit_48->setObjectName(QStringLiteral("lineEdit_48"));
        sizePolicy.setHeightForWidth(lineEdit_48->sizePolicy().hasHeightForWidth());
        lineEdit_48->setSizePolicy(sizePolicy);
        lineEdit_48->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_14->addWidget(lineEdit_48);


        verticalLayout_5->addLayout(horizontalLayout_14);

        horizontalLayout_6 = new QHBoxLayout();
        horizontalLayout_6->setSpacing(6);
        horizontalLayout_6->setObjectName(QStringLiteral("horizontalLayout_6"));
        label_14 = new QLabel(groupBox_4);
        label_14->setObjectName(QStringLiteral("label_14"));
        sizePolicy5.setHeightForWidth(label_14->sizePolicy().hasHeightForWidth());
        label_14->setSizePolicy(sizePolicy5);

        horizontalLayout_6->addWidget(label_14);

        lineEdit_13 = new QLineEdit(groupBox_4);
        lineEdit_13->setObjectName(QStringLiteral("lineEdit_13"));
        sizePolicy.setHeightForWidth(lineEdit_13->sizePolicy().hasHeightForWidth());
        lineEdit_13->setSizePolicy(sizePolicy);
        lineEdit_13->setMinimumSize(QSize(20, 0));
        lineEdit_13->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_6->addWidget(lineEdit_13);

        label_15 = new QLabel(groupBox_4);
        label_15->setObjectName(QStringLiteral("label_15"));
        sizePolicy6.setHeightForWidth(label_15->sizePolicy().hasHeightForWidth());
        label_15->setSizePolicy(sizePolicy6);

        horizontalLayout_6->addWidget(label_15);

        lineEdit_14 = new QLineEdit(groupBox_4);
        lineEdit_14->setObjectName(QStringLiteral("lineEdit_14"));
        sizePolicy.setHeightForWidth(lineEdit_14->sizePolicy().hasHeightForWidth());
        lineEdit_14->setSizePolicy(sizePolicy);
        lineEdit_14->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_6->addWidget(lineEdit_14);

        label_16 = new QLabel(groupBox_4);
        label_16->setObjectName(QStringLiteral("label_16"));
        sizePolicy6.setHeightForWidth(label_16->sizePolicy().hasHeightForWidth());
        label_16->setSizePolicy(sizePolicy6);

        horizontalLayout_6->addWidget(label_16);

        lineEdit_15 = new QLineEdit(groupBox_4);
        lineEdit_15->setObjectName(QStringLiteral("lineEdit_15"));
        sizePolicy.setHeightForWidth(lineEdit_15->sizePolicy().hasHeightForWidth());
        lineEdit_15->setSizePolicy(sizePolicy);
        lineEdit_15->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_6->addWidget(lineEdit_15);

        label_17 = new QLabel(groupBox_4);
        label_17->setObjectName(QStringLiteral("label_17"));
        sizePolicy6.setHeightForWidth(label_17->sizePolicy().hasHeightForWidth());
        label_17->setSizePolicy(sizePolicy6);

        horizontalLayout_6->addWidget(label_17);

        lineEdit_16 = new QLineEdit(groupBox_4);
        lineEdit_16->setObjectName(QStringLiteral("lineEdit_16"));
        sizePolicy.setHeightForWidth(lineEdit_16->sizePolicy().hasHeightForWidth());
        lineEdit_16->setSizePolicy(sizePolicy);
        lineEdit_16->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_6->addWidget(lineEdit_16);

        horizontalSpacer_9 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_6->addItem(horizontalSpacer_9);


        verticalLayout_5->addLayout(horizontalLayout_6);

        horizontalLayout_7 = new QHBoxLayout();
        horizontalLayout_7->setSpacing(6);
        horizontalLayout_7->setObjectName(QStringLiteral("horizontalLayout_7"));
        label_18 = new QLabel(groupBox_4);
        label_18->setObjectName(QStringLiteral("label_18"));
        sizePolicy5.setHeightForWidth(label_18->sizePolicy().hasHeightForWidth());
        label_18->setSizePolicy(sizePolicy5);

        horizontalLayout_7->addWidget(label_18);

        lineEdit_17 = new QLineEdit(groupBox_4);
        lineEdit_17->setObjectName(QStringLiteral("lineEdit_17"));
        sizePolicy.setHeightForWidth(lineEdit_17->sizePolicy().hasHeightForWidth());
        lineEdit_17->setSizePolicy(sizePolicy);
        lineEdit_17->setMinimumSize(QSize(20, 0));
        lineEdit_17->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_7->addWidget(lineEdit_17);

        label_19 = new QLabel(groupBox_4);
        label_19->setObjectName(QStringLiteral("label_19"));
        sizePolicy6.setHeightForWidth(label_19->sizePolicy().hasHeightForWidth());
        label_19->setSizePolicy(sizePolicy6);

        horizontalLayout_7->addWidget(label_19);

        lineEdit_18 = new QLineEdit(groupBox_4);
        lineEdit_18->setObjectName(QStringLiteral("lineEdit_18"));
        sizePolicy.setHeightForWidth(lineEdit_18->sizePolicy().hasHeightForWidth());
        lineEdit_18->setSizePolicy(sizePolicy);
        lineEdit_18->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_7->addWidget(lineEdit_18);

        label_20 = new QLabel(groupBox_4);
        label_20->setObjectName(QStringLiteral("label_20"));
        sizePolicy6.setHeightForWidth(label_20->sizePolicy().hasHeightForWidth());
        label_20->setSizePolicy(sizePolicy6);

        horizontalLayout_7->addWidget(label_20);

        lineEdit_19 = new QLineEdit(groupBox_4);
        lineEdit_19->setObjectName(QStringLiteral("lineEdit_19"));
        sizePolicy.setHeightForWidth(lineEdit_19->sizePolicy().hasHeightForWidth());
        lineEdit_19->setSizePolicy(sizePolicy);
        lineEdit_19->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_7->addWidget(lineEdit_19);

        label_21 = new QLabel(groupBox_4);
        label_21->setObjectName(QStringLiteral("label_21"));
        sizePolicy6.setHeightForWidth(label_21->sizePolicy().hasHeightForWidth());
        label_21->setSizePolicy(sizePolicy6);

        horizontalLayout_7->addWidget(label_21);

        lineEdit_20 = new QLineEdit(groupBox_4);
        lineEdit_20->setObjectName(QStringLiteral("lineEdit_20"));
        sizePolicy.setHeightForWidth(lineEdit_20->sizePolicy().hasHeightForWidth());
        lineEdit_20->setSizePolicy(sizePolicy);
        lineEdit_20->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_7->addWidget(lineEdit_20);

        horizontalSpacer_8 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_7->addItem(horizontalSpacer_8);


        verticalLayout_5->addLayout(horizontalLayout_7);

        SendPacketBtn_ARP = new QPushButton(groupBox_4);
        SendPacketBtn_ARP->setObjectName(QStringLiteral("SendPacketBtn_ARP"));

        verticalLayout_5->addWidget(SendPacketBtn_ARP);


        horizontalLayout_2->addWidget(groupBox_4);

        groupBox_5 = new QGroupBox(groupBox_3);
        groupBox_5->setObjectName(QStringLiteral("groupBox_5"));
        verticalLayout_8 = new QVBoxLayout(groupBox_5);
        verticalLayout_8->setSpacing(6);
        verticalLayout_8->setContentsMargins(11, 11, 11, 11);
        verticalLayout_8->setObjectName(QStringLiteral("verticalLayout_8"));
        horizontalLayout_8 = new QHBoxLayout();
        horizontalLayout_8->setSpacing(6);
        horizontalLayout_8->setObjectName(QStringLiteral("horizontalLayout_8"));
        label_22 = new QLabel(groupBox_5);
        label_22->setObjectName(QStringLiteral("label_22"));
        sizePolicy5.setHeightForWidth(label_22->sizePolicy().hasHeightForWidth());
        label_22->setSizePolicy(sizePolicy5);

        horizontalLayout_8->addWidget(label_22);

        lineEdit_21 = new QLineEdit(groupBox_5);
        lineEdit_21->setObjectName(QStringLiteral("lineEdit_21"));
        sizePolicy.setHeightForWidth(lineEdit_21->sizePolicy().hasHeightForWidth());
        lineEdit_21->setSizePolicy(sizePolicy);
        lineEdit_21->setMinimumSize(QSize(20, 0));
        lineEdit_21->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_21);

        label_23 = new QLabel(groupBox_5);
        label_23->setObjectName(QStringLiteral("label_23"));
        sizePolicy6.setHeightForWidth(label_23->sizePolicy().hasHeightForWidth());
        label_23->setSizePolicy(sizePolicy6);

        horizontalLayout_8->addWidget(label_23);

        lineEdit_22 = new QLineEdit(groupBox_5);
        lineEdit_22->setObjectName(QStringLiteral("lineEdit_22"));
        sizePolicy.setHeightForWidth(lineEdit_22->sizePolicy().hasHeightForWidth());
        lineEdit_22->setSizePolicy(sizePolicy);
        lineEdit_22->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_22);

        label_24 = new QLabel(groupBox_5);
        label_24->setObjectName(QStringLiteral("label_24"));
        sizePolicy6.setHeightForWidth(label_24->sizePolicy().hasHeightForWidth());
        label_24->setSizePolicy(sizePolicy6);

        horizontalLayout_8->addWidget(label_24);

        lineEdit_23 = new QLineEdit(groupBox_5);
        lineEdit_23->setObjectName(QStringLiteral("lineEdit_23"));
        sizePolicy.setHeightForWidth(lineEdit_23->sizePolicy().hasHeightForWidth());
        lineEdit_23->setSizePolicy(sizePolicy);
        lineEdit_23->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_23);

        label_25 = new QLabel(groupBox_5);
        label_25->setObjectName(QStringLiteral("label_25"));
        sizePolicy6.setHeightForWidth(label_25->sizePolicy().hasHeightForWidth());
        label_25->setSizePolicy(sizePolicy6);

        horizontalLayout_8->addWidget(label_25);

        lineEdit_24 = new QLineEdit(groupBox_5);
        lineEdit_24->setObjectName(QStringLiteral("lineEdit_24"));
        sizePolicy.setHeightForWidth(lineEdit_24->sizePolicy().hasHeightForWidth());
        lineEdit_24->setSizePolicy(sizePolicy);
        lineEdit_24->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_24);

        label_26 = new QLabel(groupBox_5);
        label_26->setObjectName(QStringLiteral("label_26"));
        sizePolicy6.setHeightForWidth(label_26->sizePolicy().hasHeightForWidth());
        label_26->setSizePolicy(sizePolicy6);

        horizontalLayout_8->addWidget(label_26);

        lineEdit_25 = new QLineEdit(groupBox_5);
        lineEdit_25->setObjectName(QStringLiteral("lineEdit_25"));
        sizePolicy.setHeightForWidth(lineEdit_25->sizePolicy().hasHeightForWidth());
        lineEdit_25->setSizePolicy(sizePolicy);
        lineEdit_25->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_25);

        label_27 = new QLabel(groupBox_5);
        label_27->setObjectName(QStringLiteral("label_27"));
        sizePolicy6.setHeightForWidth(label_27->sizePolicy().hasHeightForWidth());
        label_27->setSizePolicy(sizePolicy6);

        horizontalLayout_8->addWidget(label_27);

        lineEdit_26 = new QLineEdit(groupBox_5);
        lineEdit_26->setObjectName(QStringLiteral("lineEdit_26"));
        sizePolicy.setHeightForWidth(lineEdit_26->sizePolicy().hasHeightForWidth());
        lineEdit_26->setSizePolicy(sizePolicy);
        lineEdit_26->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_8->addWidget(lineEdit_26);

        horizontalSpacer_7 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_8->addItem(horizontalSpacer_7);


        verticalLayout_8->addLayout(horizontalLayout_8);

        horizontalLayout_9 = new QHBoxLayout();
        horizontalLayout_9->setSpacing(6);
        horizontalLayout_9->setObjectName(QStringLiteral("horizontalLayout_9"));
        label_28 = new QLabel(groupBox_5);
        label_28->setObjectName(QStringLiteral("label_28"));
        sizePolicy5.setHeightForWidth(label_28->sizePolicy().hasHeightForWidth());
        label_28->setSizePolicy(sizePolicy5);

        horizontalLayout_9->addWidget(label_28);

        lineEdit_27 = new QLineEdit(groupBox_5);
        lineEdit_27->setObjectName(QStringLiteral("lineEdit_27"));
        sizePolicy.setHeightForWidth(lineEdit_27->sizePolicy().hasHeightForWidth());
        lineEdit_27->setSizePolicy(sizePolicy);
        lineEdit_27->setMinimumSize(QSize(20, 0));
        lineEdit_27->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_27);

        label_29 = new QLabel(groupBox_5);
        label_29->setObjectName(QStringLiteral("label_29"));
        sizePolicy6.setHeightForWidth(label_29->sizePolicy().hasHeightForWidth());
        label_29->setSizePolicy(sizePolicy6);

        horizontalLayout_9->addWidget(label_29);

        lineEdit_28 = new QLineEdit(groupBox_5);
        lineEdit_28->setObjectName(QStringLiteral("lineEdit_28"));
        sizePolicy.setHeightForWidth(lineEdit_28->sizePolicy().hasHeightForWidth());
        lineEdit_28->setSizePolicy(sizePolicy);
        lineEdit_28->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_28);

        label_30 = new QLabel(groupBox_5);
        label_30->setObjectName(QStringLiteral("label_30"));
        sizePolicy6.setHeightForWidth(label_30->sizePolicy().hasHeightForWidth());
        label_30->setSizePolicy(sizePolicy6);

        horizontalLayout_9->addWidget(label_30);

        lineEdit_29 = new QLineEdit(groupBox_5);
        lineEdit_29->setObjectName(QStringLiteral("lineEdit_29"));
        sizePolicy.setHeightForWidth(lineEdit_29->sizePolicy().hasHeightForWidth());
        lineEdit_29->setSizePolicy(sizePolicy);
        lineEdit_29->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_29);

        label_31 = new QLabel(groupBox_5);
        label_31->setObjectName(QStringLiteral("label_31"));
        sizePolicy6.setHeightForWidth(label_31->sizePolicy().hasHeightForWidth());
        label_31->setSizePolicy(sizePolicy6);

        horizontalLayout_9->addWidget(label_31);

        lineEdit_30 = new QLineEdit(groupBox_5);
        lineEdit_30->setObjectName(QStringLiteral("lineEdit_30"));
        sizePolicy.setHeightForWidth(lineEdit_30->sizePolicy().hasHeightForWidth());
        lineEdit_30->setSizePolicy(sizePolicy);
        lineEdit_30->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_30);

        label_32 = new QLabel(groupBox_5);
        label_32->setObjectName(QStringLiteral("label_32"));
        sizePolicy6.setHeightForWidth(label_32->sizePolicy().hasHeightForWidth());
        label_32->setSizePolicy(sizePolicy6);

        horizontalLayout_9->addWidget(label_32);

        lineEdit_31 = new QLineEdit(groupBox_5);
        lineEdit_31->setObjectName(QStringLiteral("lineEdit_31"));
        sizePolicy.setHeightForWidth(lineEdit_31->sizePolicy().hasHeightForWidth());
        lineEdit_31->setSizePolicy(sizePolicy);
        lineEdit_31->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_31);

        label_33 = new QLabel(groupBox_5);
        label_33->setObjectName(QStringLiteral("label_33"));
        sizePolicy6.setHeightForWidth(label_33->sizePolicy().hasHeightForWidth());
        label_33->setSizePolicy(sizePolicy6);

        horizontalLayout_9->addWidget(label_33);

        lineEdit_32 = new QLineEdit(groupBox_5);
        lineEdit_32->setObjectName(QStringLiteral("lineEdit_32"));
        sizePolicy.setHeightForWidth(lineEdit_32->sizePolicy().hasHeightForWidth());
        lineEdit_32->setSizePolicy(sizePolicy);
        lineEdit_32->setMaximumSize(QSize(25, 16777215));

        horizontalLayout_9->addWidget(lineEdit_32);

        horizontalSpacer_6 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_9->addItem(horizontalSpacer_6);


        verticalLayout_8->addLayout(horizontalLayout_9);

        horizontalLayout_10 = new QHBoxLayout();
        horizontalLayout_10->setSpacing(6);
        horizontalLayout_10->setObjectName(QStringLiteral("horizontalLayout_10"));
        label_34 = new QLabel(groupBox_5);
        label_34->setObjectName(QStringLiteral("label_34"));
        sizePolicy5.setHeightForWidth(label_34->sizePolicy().hasHeightForWidth());
        label_34->setSizePolicy(sizePolicy5);

        horizontalLayout_10->addWidget(label_34);

        lineEdit_33 = new QLineEdit(groupBox_5);
        lineEdit_33->setObjectName(QStringLiteral("lineEdit_33"));
        sizePolicy.setHeightForWidth(lineEdit_33->sizePolicy().hasHeightForWidth());
        lineEdit_33->setSizePolicy(sizePolicy);
        lineEdit_33->setMinimumSize(QSize(20, 0));
        lineEdit_33->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_10->addWidget(lineEdit_33);

        label_35 = new QLabel(groupBox_5);
        label_35->setObjectName(QStringLiteral("label_35"));
        sizePolicy6.setHeightForWidth(label_35->sizePolicy().hasHeightForWidth());
        label_35->setSizePolicy(sizePolicy6);

        horizontalLayout_10->addWidget(label_35);

        lineEdit_34 = new QLineEdit(groupBox_5);
        lineEdit_34->setObjectName(QStringLiteral("lineEdit_34"));
        sizePolicy.setHeightForWidth(lineEdit_34->sizePolicy().hasHeightForWidth());
        lineEdit_34->setSizePolicy(sizePolicy);
        lineEdit_34->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_10->addWidget(lineEdit_34);

        label_36 = new QLabel(groupBox_5);
        label_36->setObjectName(QStringLiteral("label_36"));
        sizePolicy6.setHeightForWidth(label_36->sizePolicy().hasHeightForWidth());
        label_36->setSizePolicy(sizePolicy6);

        horizontalLayout_10->addWidget(label_36);

        lineEdit_35 = new QLineEdit(groupBox_5);
        lineEdit_35->setObjectName(QStringLiteral("lineEdit_35"));
        sizePolicy.setHeightForWidth(lineEdit_35->sizePolicy().hasHeightForWidth());
        lineEdit_35->setSizePolicy(sizePolicy);
        lineEdit_35->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_10->addWidget(lineEdit_35);

        label_37 = new QLabel(groupBox_5);
        label_37->setObjectName(QStringLiteral("label_37"));
        sizePolicy6.setHeightForWidth(label_37->sizePolicy().hasHeightForWidth());
        label_37->setSizePolicy(sizePolicy6);

        horizontalLayout_10->addWidget(label_37);

        lineEdit_36 = new QLineEdit(groupBox_5);
        lineEdit_36->setObjectName(QStringLiteral("lineEdit_36"));
        sizePolicy.setHeightForWidth(lineEdit_36->sizePolicy().hasHeightForWidth());
        lineEdit_36->setSizePolicy(sizePolicy);
        lineEdit_36->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_10->addWidget(lineEdit_36);

        horizontalSpacer_5 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_10->addItem(horizontalSpacer_5);


        verticalLayout_8->addLayout(horizontalLayout_10);

        horizontalLayout_11 = new QHBoxLayout();
        horizontalLayout_11->setSpacing(6);
        horizontalLayout_11->setObjectName(QStringLiteral("horizontalLayout_11"));
        label_38 = new QLabel(groupBox_5);
        label_38->setObjectName(QStringLiteral("label_38"));
        sizePolicy5.setHeightForWidth(label_38->sizePolicy().hasHeightForWidth());
        label_38->setSizePolicy(sizePolicy5);

        horizontalLayout_11->addWidget(label_38);

        lineEdit_37 = new QLineEdit(groupBox_5);
        lineEdit_37->setObjectName(QStringLiteral("lineEdit_37"));
        sizePolicy.setHeightForWidth(lineEdit_37->sizePolicy().hasHeightForWidth());
        lineEdit_37->setSizePolicy(sizePolicy);
        lineEdit_37->setMinimumSize(QSize(20, 0));
        lineEdit_37->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_11->addWidget(lineEdit_37);

        label_39 = new QLabel(groupBox_5);
        label_39->setObjectName(QStringLiteral("label_39"));
        sizePolicy6.setHeightForWidth(label_39->sizePolicy().hasHeightForWidth());
        label_39->setSizePolicy(sizePolicy6);

        horizontalLayout_11->addWidget(label_39);

        lineEdit_38 = new QLineEdit(groupBox_5);
        lineEdit_38->setObjectName(QStringLiteral("lineEdit_38"));
        sizePolicy.setHeightForWidth(lineEdit_38->sizePolicy().hasHeightForWidth());
        lineEdit_38->setSizePolicy(sizePolicy);
        lineEdit_38->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_11->addWidget(lineEdit_38);

        label_40 = new QLabel(groupBox_5);
        label_40->setObjectName(QStringLiteral("label_40"));
        sizePolicy6.setHeightForWidth(label_40->sizePolicy().hasHeightForWidth());
        label_40->setSizePolicy(sizePolicy6);

        horizontalLayout_11->addWidget(label_40);

        lineEdit_39 = new QLineEdit(groupBox_5);
        lineEdit_39->setObjectName(QStringLiteral("lineEdit_39"));
        sizePolicy.setHeightForWidth(lineEdit_39->sizePolicy().hasHeightForWidth());
        lineEdit_39->setSizePolicy(sizePolicy);
        lineEdit_39->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_11->addWidget(lineEdit_39);

        label_41 = new QLabel(groupBox_5);
        label_41->setObjectName(QStringLiteral("label_41"));
        sizePolicy6.setHeightForWidth(label_41->sizePolicy().hasHeightForWidth());
        label_41->setSizePolicy(sizePolicy6);

        horizontalLayout_11->addWidget(label_41);

        lineEdit_40 = new QLineEdit(groupBox_5);
        lineEdit_40->setObjectName(QStringLiteral("lineEdit_40"));
        sizePolicy.setHeightForWidth(lineEdit_40->sizePolicy().hasHeightForWidth());
        lineEdit_40->setSizePolicy(sizePolicy);
        lineEdit_40->setMaximumSize(QSize(40, 16777215));

        horizontalLayout_11->addWidget(lineEdit_40);

        horizontalSpacer_4 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_11->addItem(horizontalSpacer_4);


        verticalLayout_8->addLayout(horizontalLayout_11);

        horizontalLayout_12 = new QHBoxLayout();
        horizontalLayout_12->setSpacing(6);
        horizontalLayout_12->setObjectName(QStringLiteral("horizontalLayout_12"));
        label_42 = new QLabel(groupBox_5);
        label_42->setObjectName(QStringLiteral("label_42"));
        QSizePolicy sizePolicy7(QSizePolicy::Preferred, QSizePolicy::Minimum);
        sizePolicy7.setHorizontalStretch(0);
        sizePolicy7.setVerticalStretch(0);
        sizePolicy7.setHeightForWidth(label_42->sizePolicy().hasHeightForWidth());
        label_42->setSizePolicy(sizePolicy7);

        horizontalLayout_12->addWidget(label_42);

        lineEdit_41 = new QLineEdit(groupBox_5);
        lineEdit_41->setObjectName(QStringLiteral("lineEdit_41"));
        sizePolicy4.setHeightForWidth(lineEdit_41->sizePolicy().hasHeightForWidth());
        lineEdit_41->setSizePolicy(sizePolicy4);
        lineEdit_41->setMinimumSize(QSize(20, 0));
        lineEdit_41->setMaximumSize(QSize(60, 16777215));

        horizontalLayout_12->addWidget(lineEdit_41);

        horizontalSpacer_2 = new QSpacerItem(40, 20, QSizePolicy::Fixed, QSizePolicy::Minimum);

        horizontalLayout_12->addItem(horizontalSpacer_2);

        label_43 = new QLabel(groupBox_5);
        label_43->setObjectName(QStringLiteral("label_43"));
        sizePolicy7.setHeightForWidth(label_43->sizePolicy().hasHeightForWidth());
        label_43->setSizePolicy(sizePolicy7);

        horizontalLayout_12->addWidget(label_43);

        lineEdit_42 = new QLineEdit(groupBox_5);
        lineEdit_42->setObjectName(QStringLiteral("lineEdit_42"));
        sizePolicy4.setHeightForWidth(lineEdit_42->sizePolicy().hasHeightForWidth());
        lineEdit_42->setSizePolicy(sizePolicy4);
        lineEdit_42->setMinimumSize(QSize(20, 0));
        lineEdit_42->setMaximumSize(QSize(60, 16777215));

        horizontalLayout_12->addWidget(lineEdit_42);

        horizontalSpacer_3 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_12->addItem(horizontalSpacer_3);


        verticalLayout_8->addLayout(horizontalLayout_12);

        horizontalLayout_13 = new QHBoxLayout();
        horizontalLayout_13->setSpacing(6);
        horizontalLayout_13->setObjectName(QStringLiteral("horizontalLayout_13"));
        label_50 = new QLabel(groupBox_5);
        label_50->setObjectName(QStringLiteral("label_50"));

        horizontalLayout_13->addWidget(label_50);

        lineEdit_49 = new QLineEdit(groupBox_5);
        lineEdit_49->setObjectName(QStringLiteral("lineEdit_49"));
        sizePolicy.setHeightForWidth(lineEdit_49->sizePolicy().hasHeightForWidth());
        lineEdit_49->setSizePolicy(sizePolicy);
        lineEdit_49->setMaximumSize(QSize(100, 16777215));

        horizontalLayout_13->addWidget(lineEdit_49);

        horizontalSpacer_10 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_13->addItem(horizontalSpacer_10);

        label_51 = new QLabel(groupBox_5);
        label_51->setObjectName(QStringLiteral("label_51"));

        horizontalLayout_13->addWidget(label_51);

        lineEdit_50 = new QLineEdit(groupBox_5);
        lineEdit_50->setObjectName(QStringLiteral("lineEdit_50"));
        sizePolicy.setHeightForWidth(lineEdit_50->sizePolicy().hasHeightForWidth());
        lineEdit_50->setSizePolicy(sizePolicy);
        lineEdit_50->setMaximumSize(QSize(100, 16777215));

        horizontalLayout_13->addWidget(lineEdit_50);

        horizontalSpacer_11 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_13->addItem(horizontalSpacer_11);


        verticalLayout_8->addLayout(horizontalLayout_13);

        horizontalLayout_16 = new QHBoxLayout();
        horizontalLayout_16->setSpacing(6);
        horizontalLayout_16->setObjectName(QStringLiteral("horizontalLayout_16"));
        label_53 = new QLabel(groupBox_5);
        label_53->setObjectName(QStringLiteral("label_53"));

        horizontalLayout_16->addWidget(label_53);

        lineEdit_51 = new QLineEdit(groupBox_5);
        lineEdit_51->setObjectName(QStringLiteral("lineEdit_51"));
        lineEdit_51->setMaximumSize(QSize(60, 16777215));

        horizontalLayout_16->addWidget(lineEdit_51);

        horizontalSpacer_13 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_16->addItem(horizontalSpacer_13);

        label_55 = new QLabel(groupBox_5);
        label_55->setObjectName(QStringLiteral("label_55"));

        horizontalLayout_16->addWidget(label_55);

        lineEdit_52 = new QLineEdit(groupBox_5);
        lineEdit_52->setObjectName(QStringLiteral("lineEdit_52"));
        lineEdit_52->setMaximumSize(QSize(60, 16777215));

        horizontalLayout_16->addWidget(lineEdit_52);

        horizontalSpacer_12 = new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

        horizontalLayout_16->addItem(horizontalSpacer_12);


        verticalLayout_8->addLayout(horizontalLayout_16);

        SendPacketBtn_TCP = new QPushButton(groupBox_5);
        SendPacketBtn_TCP->setObjectName(QStringLiteral("SendPacketBtn_TCP"));

        verticalLayout_8->addWidget(SendPacketBtn_TCP);


        horizontalLayout_2->addWidget(groupBox_5);


        horizontalLayout_3->addLayout(horizontalLayout_2);


        gridLayout_4->addWidget(groupBox_3, 2, 0, 1, 1);


        gridLayout_5->addLayout(gridLayout_4, 0, 0, 1, 1);

        tabWidget->addTab(tab_2, QString());

        gridLayout_2->addWidget(tabWidget, 0, 0, 1, 1);

        MainWindowClass->setCentralWidget(centralWidget);

        retranslateUi(MainWindowClass);

        tabWidget->setCurrentIndex(0);


        QMetaObject::connectSlotsByName(MainWindowClass);
    } // setupUi

    void retranslateUi(QMainWindow *MainWindowClass)
    {
        MainWindowClass->setWindowTitle(QApplication::translate("MainWindowClass", "DXSniffer", Q_NULLPTR));
        groupBox_device->setTitle(QApplication::translate("MainWindowClass", "\350\256\276\345\244\207\345\210\227\350\241\250", Q_NULLPTR));
        submitDevice->setText(QApplication::translate("MainWindowClass", "\347\241\256\345\256\232", Q_NULLPTR));
        groupBox_packet->setTitle(QApplication::translate("MainWindowClass", "\346\215\225\350\216\267\345\210\260\347\232\204\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        tabWidget->setTabText(tabWidget->indexOf(tab), QApplication::translate("MainWindowClass", "\346\215\225\350\216\267\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        groupBox->setTitle(QApplication::translate("MainWindowClass", "\350\256\276\345\244\207\345\210\227\350\241\250", Q_NULLPTR));
        groupBox_3->setTitle(QApplication::translate("MainWindowClass", "\346\236\204\351\200\240\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        groupBox_4->setTitle(QApplication::translate("MainWindowClass", "\346\236\204\351\200\240ARP\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        label->setText(QApplication::translate("MainWindowClass", "\346\212\245\346\226\207\347\261\273\345\236\213\357\274\232", Q_NULLPTR));
        ARPTypeRB_Rq->setText(QApplication::translate("MainWindowClass", "\350\257\267\346\261\202\346\212\245\346\226\207", Q_NULLPTR));
        ARPTypeRB_Rsp->setText(QApplication::translate("MainWindowClass", "\345\272\224\347\255\224\346\212\245\346\226\207", Q_NULLPTR));
        label_2->setText(QApplication::translate("MainWindowClass", "\347\233\256\347\232\204MAC\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit->setText(QString());
        label_3->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_4->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_5->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_6->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_7->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        lineEdit_6->setText(QString());
        label_8->setText(QApplication::translate("MainWindowClass", "\346\272\220MAC\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_7->setText(QString());
        label_9->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_10->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_11->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_12->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_13->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        lineEdit_12->setText(QString());
        label_44->setText(QApplication::translate("MainWindowClass", "\347\233\256\346\240\207MAC\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_43->setText(QString());
        label_45->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_46->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_47->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_48->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_49->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        lineEdit_48->setText(QString());
        label_14->setText(QApplication::translate("MainWindowClass", "\347\233\256\347\232\204IP\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_13->setText(QString());
        label_15->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_16->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_17->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_18->setText(QApplication::translate("MainWindowClass", "\346\272\220IP\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_17->setText(QString());
        label_19->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_20->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_21->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        SendPacketBtn_ARP->setText(QApplication::translate("MainWindowClass", "\345\217\221\351\200\201ARP\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        groupBox_5->setTitle(QApplication::translate("MainWindowClass", "\346\236\204\351\200\240TCP\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        label_22->setText(QApplication::translate("MainWindowClass", "\347\233\256\347\232\204MAC\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_21->setText(QString());
        label_23->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_24->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_25->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_26->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_27->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        lineEdit_26->setText(QString());
        label_28->setText(QApplication::translate("MainWindowClass", "\346\272\220MAC\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_27->setText(QString());
        label_29->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_30->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_31->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_32->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        label_33->setText(QApplication::translate("MainWindowClass", "-", Q_NULLPTR));
        lineEdit_32->setText(QString());
        label_34->setText(QApplication::translate("MainWindowClass", "\347\233\256\347\232\204IP\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_33->setText(QString());
        label_35->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_36->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_37->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_38->setText(QApplication::translate("MainWindowClass", "\346\272\220IP\345\234\260\345\235\200\357\274\232", Q_NULLPTR));
        lineEdit_37->setText(QString());
        label_39->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_40->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_41->setText(QApplication::translate("MainWindowClass", ".", Q_NULLPTR));
        label_42->setText(QApplication::translate("MainWindowClass", "\347\233\256\347\232\204\347\253\257\345\217\243\357\274\232", Q_NULLPTR));
        lineEdit_41->setText(QString());
        label_43->setText(QApplication::translate("MainWindowClass", "\346\272\220\347\253\257\345\217\243\357\274\232", Q_NULLPTR));
        lineEdit_42->setText(QString());
        label_50->setText(QApplication::translate("MainWindowClass", "\345\272\217\345\217\267\357\274\232", Q_NULLPTR));
        label_51->setText(QApplication::translate("MainWindowClass", "\347\241\256\350\256\244\345\272\217\345\217\267\357\274\232", Q_NULLPTR));
        label_53->setText(QApplication::translate("MainWindowClass", "\347\252\227\345\217\243\345\244\247\345\260\217\357\274\232", Q_NULLPTR));
        label_55->setText(QApplication::translate("MainWindowClass", "\347\264\247\346\200\245\346\214\207\351\222\210\357\274\232", Q_NULLPTR));
        SendPacketBtn_TCP->setText(QApplication::translate("MainWindowClass", "\345\217\221\351\200\201TCP\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
        tabWidget->setTabText(tabWidget->indexOf(tab_2), QApplication::translate("MainWindowClass", "\345\217\221\351\200\201\346\225\260\346\215\256\345\214\205", Q_NULLPTR));
    } // retranslateUi

};

namespace Ui {
    class MainWindowClass: public Ui_MainWindowClass {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_MAINWINDOW_H
